...
const Cards = ({id, symbol, name, image, current_price, market_cap_rank, price_change_24h, price_change_percentage_24h}) => {
    return (
        <div className="grid grid-cols-7 pt-1">
            ...
            <p className={`mx-auto text-red ${price_change_24h > 0? "text-green-600 text-sm" : "text-red-500 text-sm "}`}>{parseFloat(price_change_24h).toFixed(3)}$</p>
            <p className={`mx-auto text-red ${price_change_percentage_24h > 0? "text-green-600 text-sm" : "text-red-500 text-sm "}`}>${price_change_percentage_24h}</p>
        </div>
    )
}
